...
class Optimizer:
    ...
    def run_all_setups(self, processes: bool = False):
        ...
        if not processes:
            ...
            self.history.to_csv(filepath)
        else:
            mp_handler = MultiprocessHandler(func=self.run_single_setup, output_file=filepath, setups=self.setups)
            mp_handler.handle()
            self.history = pd.read_csv(filepath, encoding='utf-8')
    ...
    def run_single_setup(self, setup: pd.Series, out_type: str = 'dict'):
        ...
        setup_evaluator = SetupEvaluator(model=self.clf,
                                         name="Random Forest",
                                         **recovered_dtypes
                                         )
...
class MultiprocessHandler:
    def __init__(self, func, output_file: str, setups: pd.DataFrame):
        self.func = func
    ...
    def handle(self, ):
        # fire off workers
        jobs = []
        for stp in self.setups.iterrows():
            job = self.pool.apply_async(self.worker, (stp[1], self.queue))
            jobs.append(job)
